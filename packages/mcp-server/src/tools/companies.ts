...
export function register(server: McpServer, client: JobseekClient) {
  ...
  server.tool("trigger_discovery_run", "Trigger a fresh company-discovery-actor run on Apify. Scans 33 job boards in parallel batches (Greenhouse, Lever, Ashby, Workday, SmartRecruiters, Wellfound, hiring.cafe, LinkedIn, Glassdoor, Softgarden, JOIN, etc.) and updates the dataset. Takes 10–20 min. Call get_discovery_results after completion.", { sources: z.array(z.string()).optional().describe("Override default sources list (omit to use all 33 sources)"), enableAiDiscovery: z.boolean().optional().describe("Enable Gemini AI portal discovery (default true)") }, { title: "Trigger Discovery Run", readOnlyHint: false, destructiveHint: false, openWorldHint: true }, async (p) => {

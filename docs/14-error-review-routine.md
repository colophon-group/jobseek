...
- **Preferred scheduled route:** Hetzner local Codex runner through
  ...
  sampler publishes only numeric status fields. Grafana pages on failure,
  36-hour staleness, or a run stuck over three hours. This daily routine is a
  secondary deduplicated context route, not the urgent paging dependency; see
  [independent production paging](16-hetzner-maintenance.md#independent-production-paging).

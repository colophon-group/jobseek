...
- **Graceful degradation**: company autocomplete, watchlist company search, public watchlist discovery, and watchlist posting lists return empty results when Typesense is unavailable; watchlist posting counts return zero; company detail degrades to not found. These outages are logged, outage-shaped public discovery results are not cached, and non-availability errors still propagate. No live fallback returns crawler data from Supabase
...
| Supabase Postgres | Transitional mirror of crawler data; **only home** for user-facing tables (`user`, `session`, `watchlist`, `watchlist_company`, `saved_job`, ...) | Auth, watchlist metadata/mutations, saved-job snapshots, watchlist company-pair lookups, and remaining legacy taxonomy reads pending removal; no live web reader uses its `job_posting` mirror |

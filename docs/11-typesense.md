...
| Web-owned Postgres | Transitional crawler mirror pending the protected retirement window; **only home** for user-facing tables (`user`, `session`, `watchlist`, `watchlist_company`, `saved_job`, ...) | Auth, watchlist metadata/mutations, saved-job snapshots, sitemap watchlists, and watchlist company-pair lookups; no live public web reader uses crawler-owned tables |

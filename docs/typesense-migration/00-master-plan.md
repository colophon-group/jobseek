...
- **Keyword matching**: Postgres uses OR (match ANY keyword, score by count). Typesense uses AND-first with token dropping fallback (`drop_tokens_threshold: 1`). This is an intentional improvement — more relevant top results — but NOT exact behavioral parity. Users see better results, but fewer partial matches.
- **Location geo-sorting**: Typesense's precision-bucketed geo-sort replaces the Postgres binary 300km cutoff. Nearby locations always rank above distant ones (no population fallback for far locations). This is a different model, not a bug.
- **`yearMatches` in `search()`**: Pre-computed (approximate, ~30 min) from company collection, not filtered by current keywords/filters. This is a trade-off — live filtered year counts would require a facet query that can't reliably target the same companies the relevance-ranked search returned. `activeMatches` uses `group.found` (live, exact parity). Note: `loadPostingsWithCounts()` DOES use live keyword-filtered year counts (matching Postgres behavior exactly).
- **Sort order improvements**: `getWatchlistPostings()` with keywords now sorts by relevance then recency (Postgres sorted by recency only). `searchPublicWatchlists()` sorts by text relevance then recency (Postgres sorted by recency only). Both are UX improvements.
- **Matching semantics**: `searchPublicWatchlists()` changes from ILIKE substring matching to token-based full-text search. Gains typo tolerance and prefix matching; loses mid-word substring matching. `searchCompaniesForWatchlist()` changes from LIKE prefix + pg_trgm similarity to Typesense prefix + typo tolerance. Similar trade-offs.
- **Locale-specific company descriptions**: The Typesense `company` collection has a single `description` field (not locale-aware). Users in non-English locales will see the default description instead of the localized one. This is a display-only regression (the field is `index: false`). Acceptable for initial launch; can be addressed later by adding per-locale description fields.

...
- **`yearMatches` in `search()`**: Pre-computed (approximate, ~30 min), not filtered by current keywords/filters. This is a trade-off — live filtered year counts would require a facet query that can't reliably target the same companies the relevance-ranked search returned. `activeMatches` uses `group.found` (live, exact parity with Postgres).

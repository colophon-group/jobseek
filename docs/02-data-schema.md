...
- **Rich monitors** (ashby, amazon, dvinci, gem, greenhouse, hirehive, hireology, turbohire, lever, pinpoint, recruitee, rss, traffit, wecruit, woowa; also `api_sniffer`/`nextdata` with `fields`) return full job data — `scraper_type` is empty or `skip`

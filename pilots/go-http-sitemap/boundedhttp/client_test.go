...
func TestDefaultNetworkPolicyRejectsLoopbackBeforeRequest(t *testing.T) {
	...
	session := client.NewSession()
	defer session.Close()
	_, err = session.Get(context.Background(), server.URL+"/sitemap.xml?token=secret", nil)
	if got := errorKind(t, err); got != ErrorNetworkPolicy {
		t.Fatalf("kind=%s error=%v", got, err)
	}
	if strings.Contains(fmt.Sprintf("%+v", err), "secret") || requests.Load() != 0 {
		t.Fatalf("error=%+v requests=%d", err, requests.Load())
	}
	if stats := session.Stats(); stats.Requests != 1 || stats.WireAttempts != 0 {
		t.Fatalf("stats=%+v", stats)
	}
}

func TestPublicNetworkPolicyRejectsUnsafeAddressClasses(t *testing.T) {
	for _, raw := range []string{
		"0.0.0.0", "10.0.0.1", "100.64.0.1", "127.0.0.1", "169.254.169.254",
		"192.0.2.1", "198.18.0.1", "198.51.100.1", "203.0.113.1", "224.0.0.1", "240.0.0.1",
		"::", "::1", "::127.0.0.1", "::ffff:127.0.0.1", "64:ff9b::a9fe:a9fe", "64:ff9b:1::a9fe:a9fe",
		"100::1", "2001::1", "2001:2::1", "2001:db8::1", "2002:a9fe:a9fe::1", "3fff::1", "fc00::1", "fec0::1", "fe80::1", "ff02::1",
	} {
	...
	for _, raw := range []string{"8.8.8.8", "1.1.1.1", "2606:4700:4700::1111"} {
		...
		if !isPublicAddress(ip) {
			t.Fatalf("public address rejected: %s", ip)
		}
	}
}

func TestPublicNetworkDialPinsValidatedDNSAndRejectsMixedAnswers(t *testing.T) {
	client, err := New(Config{
		RequestTimeout:           time.Second,
		MaxDecodedBodyBytes:      1,
		MaxRequests:              1,
	...
	dial := client.publicOnlyDialContext(func(_ context.Context, _, address string) (net.Conn, error) {
		dialed = address
		left, right := net.Pipe()
		go func() {
			_ = right.Close()
	...
	conn, err := dial(context.Background(), "tcp", "jobs.example:443")
	if err != nil {
		t.Fatal(err)
	}
	_ = conn.Close()
	...
	for name, lookup := range map[string]func(context.Context, string, string) ([]net.IP, error){
		"mixed": func(context.Context, string, string) ([]net.IP, error) {
			return []net.IP{net.ParseIP("93.184.216.34"), net.ParseIP("127.0.0.1")}, nil
		},
		"private": func(context.Context, string, string) ([]net.IP, error) {
			return []net.IP{net.ParseIP("10.0.0.1")}, nil
		},
		"mapped private": func(context.Context, string, string) ([]net.IP, error) {
			return []net.IP{net.ParseIP("::ffff:169.254.169.254")}, nil
		},
		"resolver failure": func(context.Context, string, string) ([]net.IP, error) {
			return nil, errors.New("resolver secret")
		},
	} {
		t.Run(name, func(t *testing.T) {
			client.lookupIP = lookup
			called := false
			dial := client.publicOnlyDialContext(func(context.Context, string, string) (net.Conn, error) {
				called = true
				return nil, errors.New("must not dial")
			})
			_, err := dial(context.Background(), "tcp", "jobs.example:443")
			var policyErr *networkPolicyError
			if !errors.As(err, &policyErr) || called {
				t.Fatalf("error=%v dial_called=%t", err, called)
			}
			session := client.NewSession()
			defer session.Close()
			_, err = session.Get(context.Background(), "https://jobs.example/sitemap.xml?token=secret", nil)
			if got := errorKind(t, err); got != ErrorNetworkPolicy {
				t.Fatalf("kind=%s error=%v", got, err)
			}
...
func TestTDMReservationHeaderStopsBeforeBodyOrStatusHandling(t *testing.T) {
	for _, status := range []int{http.StatusOK, http.StatusInternalServerError} {
		t.Run(http.StatusText(status), func(t *testing.T) {
			...
			session := testClient(t, nil).NewSession()
			defer session.Close()
			_, err := session.Get(context.Background(), server.URL+"/sitemap.xml?token=request", nil)
			if got := errorKind(t, err); got != ErrorTDMReservation {
				t.Fatalf("kind=%s error=%v", got, err)
			}
			printed := fmt.Sprintf("%+v", err)
			if strings.Contains(printed, "request") || strings.Contains(printed, "companion") || strings.Contains(printed, "policy.example") {
				t.Fatalf("secret-bearing error=%s", printed)
			}
...
func TestTDMReservationZeroAndMalformedValuesAllowBody(t *testing.T) {
	for name, values := range map[string][]string{
		"zero":        {"0"},
		"word":        {"true"},
		"semicolon":   {"1; policy=x"},
		"multi value": {"1", "0"},
	} {
		t.Run(name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
				for _, value := range values {
					w.Header().Add("TDM-Reservation", value)
				}
				_, _ = w.Write([]byte("ok"))
			}))
			defer server.Close()

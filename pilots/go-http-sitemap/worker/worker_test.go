...
func TestSitemapProcessorUsesSharedHTTPClientAcrossJobs(t *testing.T) {
	...
	first := testJob("one", server.URL)
	first.Sitemap.AllowHTTPForTesting = true
	second := testJob("two", server.URL)
	second.Sitemap.AllowHTTPForTesting = true
	submit(t, pool, first)
	submit(t, pool, second)
	pool.Close()
	results := collectResults(t, pool)
	for _, result := range results {

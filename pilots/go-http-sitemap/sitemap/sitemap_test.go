...
func newRunner(t *testing.T, client *boundedhttp.Client, config Config) *Runner {
	t.Helper()
	if strings.HasPrefix(config.SitemapURL, "http://") {
...
func TestChildFetchIsConfinedToExactRootOrigin(t *testing.T) {
	...
	root := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = fmt.Fprintf(w, `<sitemapindex><sitemap><loc>%s/child.xml?token=secret</loc></sitemap></sitemapindex>`, other.URL)
	}))
	defer root.Close()

	runner := newRunner(t, newHTTPClient(t, 2), Config{
		SitemapURL:       root.URL + "/index.xml",
		MaxURLs:          50_000,
		MaxIndexChildren: 1,
		RootMaxAttempts:  1,
	})
	...
	if !errors.As(err, &sitemapErr) || sitemapErr.Kind != ErrorOrigin {
		t.Fatalf("result=%+v error=%v", result, err)
	}
	if crossOriginRequests.Load() != 0 || strings.Contains(fmt.Sprintf("%+v", err), "secret") {
		t.Fatalf("cross-origin requests=%d error=%+v", crossOriginRequests.Load(), err)
	}
	if result.TransportMetrics.Requests != 1 || result.TransportMetrics.WireAttempts != 1 {
		t.Fatalf("metrics=%+v", result.TransportMetrics)
	}
}

func TestExactOriginCanonicalizationIsFailClosed(t *testing.T) {
	root, err := url.Parse("https://EXAMPLE.com:443/sitemap.xml")
	if err != nil {
		t.Fatal(err)
	}
	if got := canonicalOrigin(root); got != "https://example.com" {
		t.Fatalf("canonical root=%q", got)
	}
	for raw, same := range map[string]bool{
		"https://example.com/child.xml":         true,
		"https://EXAMPLE.COM:443/child.xml":     true,
		"https://example.com:444/child.xml":     false,
		"http://example.com/child.xml":          false,
		"https://example.com./child.xml":        false,
		"https://xn--xample-9ua.com/child.xml":  false,
		"https://éxample.com/child.xml":         false,
		"https://user:secret@example.com/a.xml": false,
	} {
		parsed, parseErr := url.Parse(raw)
		got := parseErr == nil && parsed.User == nil && canonicalOrigin(parsed) == canonicalOrigin(root)
		if got != same {
			t.Fatalf("same-origin(%q)=%t parse_error=%v", raw, got, parseErr)
		}
	}
}
...
func TestProductionConfigRequiresHTTPS(t *testing.T) {
	...
	config := Config{SitemapURL: "http://example.com/sitemap.xml", MaxURLs: 1, MaxIndexChildren: 1}
	if _, err := New(client, config); err == nil {
		t.Fatal("expected HTTP root rejection")
	}
	config.AllowHTTPForTesting = true
	if _, err := New(client, config); err != nil {
		t.Fatalf("test-only HTTP opt-in rejected: %v", err)
	}
}

func TestPrivateRootIsRejectedOnceWithoutWireAttempt(t *testing.T) {
	client, err := boundedhttp.New(boundedhttp.Config{
	...
	runner, err := New(client, Config{
		SitemapURL:       "https://169.254.169.254/latest/meta-data/?token=secret",
		MaxURLs:          50_000,
		MaxIndexChildren: 1,
		RootMaxAttempts:  3,
		RootBackoff:      time.Millisecond,
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if !errors.As(err, &boundedErr) || boundedErr.Kind != boundedhttp.ErrorNetworkPolicy {
		t.Fatalf("result=%+v error=%v", result, err)
	}
	if strings.Contains(fmt.Sprintf("%+v", err), "secret") || result.TransportMetrics.Requests != 1 || result.TransportMetrics.WireAttempts != 0 {
		t.Fatalf("result=%+v error=%+v", result, err)
	}
}
...
func TestTDMReservationIsTypedAndNeverRetriedForRootOrChild(t *testing.T) {
	t.Run("root", func(t *testing.T) {
		var requests atomic.Int32
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			requests.Add(1)
			w.Header().Set("TDM-Reservation", "1")
			w.Header().Set("TDM-Policy", "https://policy.example/?secret=companion")
			_, _ = w.Write([]byte(`<urlset><url><loc>https://example.test/jobs/1</loc></url></urlset>`))
		}))
		defer server.Close()

		runner := newRunner(t, newHTTPClient(t, 3), Config{
			SitemapURL:       server.URL + "/sitemap.xml?token=request",
			MaxURLs:          50_000,
			MaxIndexChildren: 1,
			RootMaxAttempts:  3,
			RootBackoff:      time.Millisecond,
		...
		result, err := runner.Run(context.Background())
		var boundedErr *boundedhttp.Error
		if !errors.As(err, &boundedErr) || boundedErr.Kind != boundedhttp.ErrorTDMReservation {
			t.Fatalf("result=%+v error=%v", result, err)
		}
		printed := fmt.Sprintf("%+v", err)
		if requests.Load() != 1 || strings.Contains(printed, "request") || strings.Contains(printed, "companion") {
			t.Fatalf("requests=%d error=%s", requests.Load(), printed)
		}
		if result.TransportMetrics.Requests != 1 || result.TransportMetrics.DecodedBytes != 0 || result.TransportMetrics.StatusBodyBytes != 0 {
			t.Fatalf("metrics=%+v", result.TransportMetrics)
		}
	})
	...
	t.Run("child", func(t *testing.T) {
		...
		runner := newRunner(t, newHTTPClient(t, 3), Config{
			SitemapURL:       server.URL + "/index.xml",
			MaxURLs:          50_000,
			MaxIndexChildren: 1,
			RootMaxAttempts:  3,
		})
		...
		if rootRequests.Load() != 1 || childRequests.Load() != 1 || result.TransportMetrics.Requests != 2 || result.TransportMetrics.DecodedBytes == 0 {
			t.Fatalf("root=%d child=%d metrics=%+v", rootRequests.Load(), childRequests.Load(), result.TransportMetrics)
		}
	})
}

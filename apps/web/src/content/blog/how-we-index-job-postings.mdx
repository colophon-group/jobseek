...
The output is a `JobContent` record with normalized fields: an employment type that's one of six enums (`full_time`, `part_time`, `contract`, `temporary`, `volunteer`, `full_or_part`), a job-location-type from a set of three (`onsite`, `remote`, `hybrid`), locations resolved to [GeoNames](https://www.geonames.org/) IDs, and a salary parsed into a structured currency-min-max with frequency. Internships are represented by the Intern level, not by employment type. The thing that arrives in our database is more uniform than the careers page it came from. Without that uniformity, comparing roles across companies is hopeless.

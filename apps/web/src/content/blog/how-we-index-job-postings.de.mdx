...
Das Ergebnis ist ein `JobContent`-Datensatz mit normalisierten Feldern: ein Beschäftigungstyp aus sechs Enums (`full_time`, `part_time`, `contract`, `temporary`, `volunteer`, `full_or_part`), ein Job-Location-Type aus einer Dreiermenge (`onsite`, `remote`, `hybrid`), Orte aufgelöst zu [GeoNames](https://www.geonames.org/)-IDs und ein Gehalt geparst zu einem strukturierten Currency-Min-Max mit Frequenz. Praktika werden über die Stufe Intern abgebildet, nicht über den Beschäftigungstyp. Was in unserer Datenbank ankommt, ist einheitlicher als die Karriereseite, von der es stammt. Ohne diese Einheitlichkeit ist der Vergleich von Rollen über Unternehmen hinweg aussichtslos.

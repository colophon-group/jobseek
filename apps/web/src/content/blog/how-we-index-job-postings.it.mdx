...
L'output è un record `JobContent` con campi normalizzati: un tipo di impiego scelto tra sei enum (`full_time`, `part_time`, `contract`, `temporary`, `volunteer`, `full_or_part`), un job-location-type da un set di tre (`onsite`, `remote`, `hybrid`), località risolte in ID [GeoNames](https://www.geonames.org/) e una retribuzione parsata in currency-min-max strutturato con frequenza. I tirocini sono rappresentati dal livello Intern, non dal tipo di impiego. Ciò che arriva nel nostro database è più uniforme della pagina carriere da cui proviene. Senza quell'uniformità, confrontare ruoli tra aziende è senza speranza.

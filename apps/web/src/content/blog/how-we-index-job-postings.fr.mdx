...
Le résultat est un enregistrement `JobContent` aux champs normalisés : un type d'emploi parmi six enums (`full_time`, `part_time`, `contract`, `temporary`, `volunteer`, `full_or_part`), un job-location-type parmi trois (`onsite`, `remote`, `hybrid`), des localisations résolues vers des IDs [GeoNames](https://www.geonames.org/) et un salaire parsé en currency-min-max structuré avec fréquence. Les stages sont représentés par le niveau Intern, et non par le type d'emploi. Ce qui arrive dans notre base est plus uniforme que la page carrière dont c'est issu. Sans cette uniformité, comparer des rôles entre entreprises est sans espoir.

...
new completion marker only after the full matrix succeeds.
...
6. Fails the run if any card or the completion marker cannot be uploaded.
...
6. After the completion marker exists, company metadata points directly to
   public R2. Its source-version query key cannot inherit a public 404 cached
   before the object was uploaded, and the OG Function is no longer invoked.
...
prewarm therefore preserves the existing social preview behavior.

...
class TestDomDiscoverInitialFetch:
    ...
    async def test_jsonld_verification_enforces_hiring_organization(self):
        board_url = "https://jobs.example.com/company"
        company_url = "https://jobs.example.com/profile/company-role"
        partner_url = "https://jobs.example.com/profile/partner-role"
        missing_url = "https://jobs.example.com/profile/missing-employer"
        listing = _html_with_links(company_url, partner_url, missing_url)

        def jobposting(name: str | None) -> str:
            organization = (
                f',"hiringOrganization":{{"@type":"Organization","name":"{name}"}}'
                if name is not None
                else ""
            )
            return (
        ...
        pages = {
            board_url: listing,
            company_url: jobposting("Example Hospital"),
            partner_url: jobposting("Independent Partner"),
            missing_url: jobposting(None),
        }
    ...
    @pytest.mark.parametrize(
        ...
        [
            {},
            {"hiring_organization": ""},
            {"hiring_organization": "[invalid"},
            {"hiring_organization": "Example", "extra": True},
        ],
    )
    async def test_rejects_invalid_jsonld_hiring_organization_filter(self, config):
        with pytest.raises(ValueError, match="require_jsonld_jobposting"):
            await dom_discover(
                {
                    "board_url": "https://jobs.example.com/company",
                    "metadata": {"require_jsonld_jobposting": config},
                },
                AsyncMock(),
            )

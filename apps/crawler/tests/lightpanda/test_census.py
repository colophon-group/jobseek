...
def test_committed_manifest_is_current_and_contains_kpmg_fallback() -> None:
    ...
    assert manifest["summary"]["browser_board_count"] == 477
    assert manifest["summary"]["browser_required_step_count"] == 617
    assert manifest["summary"]["configured_profile_occurrence_count"] == 619
    assert any(

...
const endpoint = "https://jobs.booking.com/api/jobs?page=1&sortBy=relevance&descending=false&internal=false&tags1=Booking.com%20Company%20Hierarchy%7CTransport%20Company%20Hierarchy"
...
func publicDialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	if len(answers) == 0 {
		return nil, errors.New("Booking host has no DNS addresses")
	}
	for _, answer := range answers {
		ip, valid := netip.AddrFromSlice(answer.IP)
		if !valid || !publicAddress(ip.Unmap()) {
			return nil, errors.New("Booking host resolved to a non-public address")
		}
	}
	...
	return nil, errors.New("Booking host has no reachable public address")
}
...
func Fetch(ctx context.Context, client requestDoer) (FetchResult, error) {
	...
	if len(body) > maxResponseBytes {
		return result, errors.New("Booking response exceeded 2 MiB")
	}
	...
	if response.StatusCode != 200 {
		return result, fmt.Errorf("Booking returned HTTP %d", response.StatusCode)
	}

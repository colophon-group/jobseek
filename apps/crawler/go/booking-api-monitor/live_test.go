...
func TestLiveClientNegotiatesHTTP2WithCustomDialer(t *testing.T) {
	server := httptest.NewUnstartedServer(http.HandlerFunc(func(w http.ResponseWriter, request *http.Request) {
		...
		_, _ = io.WriteString(w, `{"jobs":[],"totalCount":0}`)
	}))
	...
	result, err := Fetch(context.Background(), client)
...
func TestFetchSingleResponseAndHTTPError(t *testing.T) {
	client := &fakeClient{status: 200, body: `{"jobs":[],"totalCount":0}`, header: http.Header{}}
	result, err := Fetch(context.Background(), client)
	if err != nil || client.calls != 1 || result.Requests != 1 || result.Responses != 1 ||
		result.Status != 200 || result.FinalURL != endpoint {
		t.Fatalf("unexpected success: %#v, %v", result, err)
	}
	client = &fakeClient{status: 404, body: "gone", header: http.Header{}}
	result, err = Fetch(context.Background(), client)
	if err == nil || result.ErrorKind != "" || len(result.URLs) != 0 || client.calls != 1 || result.Status != 404 {
		t.Fatalf("unexpected HTTP error: %#v, %v", result, err)
	}
}

func TestFetchHonorsTDMHeaderBeforeBody(t *testing.T) {
	client := &fakeClient{status: 200, body: `{"jobs":[],"totalCount":0}`, header: http.Header{"Tdm-Reservation": {"1"}}}
	result, err := Fetch(context.Background(), client)
	if err == nil || result.ErrorKind != "tdm" || result.Bytes != 0 || len(result.URLs) != 0 {

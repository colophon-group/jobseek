...
func publicDialContext(ctx context.Context, network, address string) (net.Conn, error) {
	...
	if len(answers) == 0 {
		return nil, errors.New("Pinpoint host has no DNS addresses")
	}
	for _, answer := range answers {
		ip, valid := netip.AddrFromSlice(answer.IP)
		if !valid || !publicAddress(ip.Unmap()) {
			return nil, errors.New("Pinpoint host resolved to a non-public address")
		}
	}
	...
	return nil, errors.New("Pinpoint host has no reachable public address")
}

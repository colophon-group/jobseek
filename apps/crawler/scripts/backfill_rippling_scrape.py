"""Backfill next_scrape_at for active Rippling postings.

After converting the Rippling monitor from N+1 (rich) to URL-only,
existing active postings need next_scrape_at set so the scraper picks
them up on the daily schedule.

Usage:
  uv run python scripts/backfill_rippling_scrape.py --dry-run
  uv run python scripts/backfill_rippling_scrape.py
"""
...
WHERE is_active = true AND next_scrape_at IS NULL
  AND board_id IN (SELECT id FROM job_board WHERE crawler_type = 'rippling')
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Backfill next_scrape_at for active Rippling postings"
    )
...
async def _run() -> int:
    ...
    try:
        ...
        log.info("backfill.preview", active_rippling_null_scrape=count)

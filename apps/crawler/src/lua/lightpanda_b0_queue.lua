...
local function producer_owner_valid(require_arguments)
    ...
    if cohort ~= "c1" and cohort ~= "c2" and cohort ~= "c3" and cohort ~= "c4" then return false end
    local seen = {}
    local slugs = {}
...
local function create_producer_owner()
    ...
    if (ARGV[21] ~= "c1" and ARGV[21] ~= "c2" and ARGV[21] ~= "c3" and ARGV[21] ~= "c4") or not count or count > 16 then
        return false
    end
    local previous = nil
    local fields = {
...
local function rollback_tombstone_arguments_valid()
    return engine_owner == "go" and safe_identifier(ARGV[18])
        and (ARGV[21] == "c1" or ARGV[21] == "c2" or ARGV[21] == "c3" or ARGV[21] == "c4")

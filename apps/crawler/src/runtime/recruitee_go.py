"""Default-off exclusive Go Recruitee rich monitor for direct tenant boards."""
...
def _eligible(board_url: str, monitor_type: str, config: dict | None, pw: object | None) -> str:
    ...
    if not hostname.endswith(".recruitee.com"):
        raise ValueError("Go Recruitee requires a direct hosted tenant")
    tenant = hostname.removesuffix(".recruitee.com")
    try:
        port = parsed.port
    except ValueError as exc:
        raise ValueError("Go Recruitee requires a canonical hosted URL") from exc
    if (
        ...
        or metadata.get("api_base", f"https://{tenant}.recruitee.com")
        != f"https://{tenant}.recruitee.com"
        or metadata.get("scraper_type") != "skip"
        or set(metadata) - {"slug", "api_base"} - _IGNORED - _BOOKKEEPING
    ):
        raise ValueError("Go Recruitee requires an unchanged hosted rich configuration")
    return tenant


def percentage_selected(board_id: str, board_url: str, config: dict | None) -> bool:
    raw = os.environ.get("RECRUITEE_GO_PERCENT", "0")
        ...
        *,
        ...
        try:
            ...
            if len(stdout) > 80_000_000:
                raise ValueError("Go Recruitee output exceeded its selected origin bound")
            ...
            if proc.returncode != 0 or payload.get("error"):
                ...
                if error_kind == "gone" and status == 404:
                    raise BoardGoneError(
                        f"Recruitee tenant {tenant!r} returned 404",
                        url=api_url,
                        status_code=404,
                    )
                if responses:

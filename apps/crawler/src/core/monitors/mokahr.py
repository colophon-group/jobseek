...
async def discover(board: dict, client: httpx.AsyncClient, pw=None) -> list[DiscoveredJob]:
    ...
    path = m.group(1) if m else ("campus-recruitment" if "campus" in board_url else "social-recruitment")
...
async def can_handle(url: str, client: httpx.AsyncClient | None = None, pw=None) -> dict | None:
    ...
    m = re.search(r"app\.mokahr\.com/(?:social|campus)[_-](?:recruitment|apply)/([\w-]+)/(\d+)", url)
    if not m:
        return None

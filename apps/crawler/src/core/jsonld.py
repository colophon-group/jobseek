...
def _extract_meta_locations(meta: dict[str, str]) -> list[str] | None:
    ...
    for key in ("gtm_tbcn_location", "dimension7"):
        candidate_locations = _normalize_meta_locations(meta.get(key))
...
def parse_rendered_html(url: str, config: dict | None, html: str) -> JobContent:
    """Parse already-rendered HTML through the canonical JSON-LD path."""

    parser_config = config or {}
    return _apply_defaults_by_url(parse_html(html, parser_config), parser_config, url)

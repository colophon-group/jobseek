...
`source_identity`. Omission or JSON null means legacy canonical-URL identity;
a present string must satisfy the frozen current-v1 source-identity registry.
Each batch is exactly `{checked_count: uint64, complete: boolean, result}`.
...
`job_effects[i]` may contain optional `source_identity` at its frozen v1
location. Omission and JSON null are legacy-equivalent. A present value must
pass the same registry and collision rules as a discovered job. The other
three aligned arrays never contain source identity.
...
Scrape projection remains identity-absent; source identity is accepted only
on monitor discovered jobs and projected monitor `JobEffect` values.
...
Build a complete `(canonical URL, identity state/value, canonical content or
URL-only)` tuple before sorting. A URL-only entry and legacy rich job with the
same source URL produce one rich tuple. Byte-identical rich duplicates with
the same identity state/value deduplicate. The same present identity on
different canonical URLs, the same canonical URL with different present
identities, mixing absent and present identity for one canonical URL,
different source spellings that canonicalize to one target, or divergent rich
content for one source URL suppresses the whole monitor as
`canonical_collision`. URL-only targets carry no source identity.

Sort complete tuples by canonical URL UTF-8 bytes, then identity state with
absent before present, then present identity UTF-8 bytes. Derive `targets`,
...
`source_identity` for an explicitly identified rich monitor job. Each target
is exactly `{url, action}` plus `content_sha256` for rich content. The four target

...
func semanticsValidateExistingEffects(value any) *semanticFailure {
	...
	if _, present := effects["source_identity"]; present {
		return semanticsRejected("invalid_projection")
	}
	...
	for index, rawURL := range urls {
		...
		job, jobFailure := semanticsShape(
			jobs[index],
			map[string]bool{"content_sha256": true, "source_url": true},
			map[string]bool{"source_identity": true},
		)
		target, targetOK := targets[index].(map[string]any)
		if jobFailure != nil || !digestOK || !targetOK || job["source_url"] != canonical ||
			job["content_sha256"] != digest || target["url"] != canonical {
			return semanticsRejected("invalid_projection")
		}
		if _, present := target["source_identity"]; present {
			return semanticsRejected("invalid_projection")
		}
		identityValue, identityPresent := job["source_identity"]
		identity, identityFailure := semanticsSourceIdentity(identityValue, identityPresent)
		...
		if previous, duplicate := seenURLs[canonical]; duplicate &&
			(previous.identity != identity || previous.digest != digest) {
			return semanticsFailure("canonical_collision")
		}
		if identity != "" {
			if previousURLForIdentity, duplicate := seenIdentities[identity]; duplicate && previousURLForIdentity != canonical {
				return semanticsFailure("canonical_collision")
			}
			seenIdentities[identity] = canonical
		}
		seenURLs[canonical] = struct {
			identity string
			digest   string
		}{identity: identity, digest: digest}
		if index > 0 && (canonical < previousURL ||
			(canonical == previousURL && previousPresent && identity == "") ||
			(canonical == previousURL && previousPresent == (identity != "") && identity < previousIdentity)) {
			return semanticsRejected("invalid_projection")
		}
		previousURL, previousIdentity, previousPresent = canonical, identity, identity != ""
	}
	return nil
}

func semanticsSourceIdentity(value any, present bool) (string, *semanticFailure) {
...
func semanticsProjectMonitor(input map[string]any) (map[string]any, *semanticFailure) {
	...
	identityURLs := map[string]string{}
	...
	for _, result := range results {
		...
		for _, sourceValue := range urls {
			...
			if previous, duplicate := tuples[canonical]; duplicate && previous.sourceIdentity != "" {
				return nil, semanticsFailure("canonical_collision")
			}
		...
		for _, discoveredValue := range jobs {
			...
			identityValue, identityPresent := discovered["source_identity"]
			sourceIdentity, identityFailure := semanticsSourceIdentity(identityValue, identityPresent)
			if identityFailure != nil {
				return nil, identityFailure
			}
			if sourceIdentity != "" {
				if previousURL, duplicate := identityURLs[sourceIdentity]; duplicate && previousURL != canonical {
					return nil, semanticsFailure("canonical_collision")
				}
			}
			...
			tuples[canonical] = semanticTargetTuple{
				source: source, content: content, sourceIdentity: sourceIdentity,
			}
			if sourceIdentity != "" {
				identityURLs[sourceIdentity] = canonical
			}
		}
	...
	for _, canonical := range canonicalURLs {
		...
		semanticsAddTarget(effects, canonical, &digest, tuple.sourceIdentity)
	}
	return effects, nil
}

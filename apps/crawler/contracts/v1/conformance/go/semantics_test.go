...
func semanticsCloneCaseForTest(t *testing.T, item map[string]any) map[string]any {
	t.Helper()
...
func TestSemanticsRequiredIDsAreHardCodedAndComplete(t *testing.T) {
	...
	if len(manifest.Cases) != 54 {
		t.Fatalf("semantics case count = %d, want 54", len(manifest.Cases))
	}
...
func TestSemanticsManifestMatchesEveryExactGoResultAndDigest(t *testing.T) {
	...
	if projected != 19 {
		t.Fatalf("projected result count = %d, want 19", projected)
	}
}
...
func TestSemanticsSourceIdentityLegacyStabilityAndSensitivity(t *testing.T) {
	manifest := loadSemanticsManifest(t)
	explicitNull := semanticsCloneCaseForTest(
		t, semanticsCaseByID(t, manifest, "source_identity_explicit_null_legacy_equivalence"),
	)
	expectedNull := explicitNull["expected"].(map[string]any)
	nullEffects := expectedNull["projected_effects"].(map[string]any)
	nullJob := nullEffects["job_effects"].([]any)[0].(map[string]any)
	if _, present := nullJob["source_identity"]; present {
		t.Fatal("explicit null source identity changed legacy JobEffect bytes")
	}
	input := explicitNull["input"].(map[string]any)
	result := input["result"].(map[string]any)
	delete(result["jobs"].([]any)[0].(map[string]any), "source_identity")
	existing := input["existing_projected_effects"].(map[string]any)
	delete(existing["job_effects"].([]any)[0].(map[string]any), "source_identity")
	actualNull := ProjectSemantics(explicitNull)
	if !bytes.Equal(
		semanticsJSONForTest(t, map[string]any(actualNull)),
		semanticsJSONForTest(t, expectedNull),
	) {
		t.Fatal("omitted and explicit-null identities are not legacy equivalent")
	}

	present := semanticsCloneCaseForTest(
		t,
		semanticsCaseByID(
			t, manifest, "source_identity_valid_present_propagation_and_semantic_sensitivity",
		),
	)
	presentExpected := present["expected"].(map[string]any)
	presentEffects := presentExpected["projected_effects"].(map[string]any)
	presentJob := presentEffects["job_effects"].([]any)[0].(map[string]any)
	if presentJob["source_identity"] != "smartrecruiters:example:42" {
		t.Fatalf("source identity did not propagate: %v", presentJob)
	}
	if !reflect.DeepEqual(presentEffects["content_hashes"], nullEffects["content_hashes"]) {
		t.Fatal("source identity changed the content hash")
	}
	presentInput := present["input"].(map[string]any)
	presentResult := presentInput["result"].(map[string]any)
	presentResult["jobs"].([]any)[0].(map[string]any)["source_identity"] =
		"smartrecruiters:example:43"
	changed := ProjectSemantics(present)
	changedEffects := changed["projected_effects"].(map[string]any)
	if !reflect.DeepEqual(changedEffects["content_hashes"], presentEffects["content_hashes"]) {
		t.Fatal("changing source identity changed content hashes")
	}
	if changed["semantic_sha256"] == presentExpected["semantic_sha256"] {
		t.Fatal("changing source identity did not change the semantic digest")
	}

	for _, caseID := range []string{"safe_scrape_projected", "safe_monitor_url_only"} {
		fixture := semanticsCaseByID(t, manifest, caseID)
		effects := fixture["expected"].(map[string]any)["projected_effects"].(map[string]any)
		for _, effectValue := range effects["job_effects"].([]any) {
			if _, present := effectValue.(map[string]any)["source_identity"]; present {
				t.Fatalf("%s unexpectedly projected source identity", caseID)
			}
		}
	}
}

func TestSemanticsSourceIdentityInvalidValuesReject(t *testing.T) {
	manifest := loadSemanticsManifest(t)
	for _, invalid := range []any{42, "", "not-namespaced", "SmartRecruiters:example:42"} {
		fixture := semanticsCloneCaseForTest(
			t,
			semanticsCaseByID(
				t, manifest, "source_identity_valid_present_propagation_and_semantic_sensitivity",
			),
		...
		result := input["result"].(map[string]any)
		result["jobs"].([]any)[0].(map[string]any)["source_identity"] = invalid
		actual := ProjectSemantics(fixture)
		if actual["status"] != "rejected" || actual["reason"] != "invalid_projection" {
			t.Fatalf("invalid source identity %v produced %v", invalid, actual)
		}
	}
}

func TestSemanticsSourceIdentityCollisionsAndLocations(t *testing.T) {
	manifest := loadSemanticsManifest(t)
	collision := semanticsCaseByID(t, manifest, "source_identity_collision_rejected")
	collisionExpected := collision["expected"].(map[string]any)
	if collisionExpected["status"] != "suppressed" ||
		collisionExpected["reason"] != "canonical_collision" {
		t.Fatalf("source identity collision is not closed: %v", collisionExpected)
	}

	newBase := func() map[string]any {
		return semanticsCloneCaseForTest(
			t,
			semanticsCaseByID(
				t, manifest, "source_identity_valid_present_propagation_and_semantic_sensitivity",
			),
	...
	jobsFor := func(fixture map[string]any) []any {
		input := fixture["input"].(map[string]any)
		return input["result"].(map[string]any)["jobs"].([]any)
	}

	duplicate := newBase()
	jobs := jobsFor(duplicate)
	cloneValue, err := semanticsClone(jobs[0])
	if err != nil {
		t.Fatal(err)
	}
	duplicate["input"].(map[string]any)["result"].(map[string]any)["jobs"] = append(
		jobs, cloneValue,
	)
	if ProjectSemantics(duplicate)["status"] != "projected" {
		t.Fatal("identical source identity duplicate did not deduplicate")
	}

	different := semanticsCloneCaseForTest(t, duplicate)
	jobsFor(different)[1].(map[string]any)["source_identity"] = "smartrecruiters:example:43"
	if ProjectSemantics(different)["reason"] != "canonical_collision" {
		t.Fatal("same URL with different identities did not collide")
	}

	absent := semanticsCloneCaseForTest(t, duplicate)
	delete(jobsFor(absent)[1].(map[string]any), "source_identity")
	if ProjectSemantics(absent)["reason"] != "canonical_collision" {
		t.Fatal("mixed absent/present identity did not collide")
	}

	mixedURL := newBase()
	mixedInput := mixedURL["input"].(map[string]any)
	mixedResult := mixedInput["result"].(map[string]any)
	mixedResult["urls"] = []any{jobsFor(mixedURL)[0].(map[string]any)["url"]}
	if ProjectSemantics(mixedURL)["reason"] != "canonical_collision" {
		t.Fatal("URL-only and explicit identity for one URL did not collide")
	}

	misplaced := newBase()
	misplacedInput := misplaced["input"].(map[string]any)
	misplacedExisting := misplacedInput["existing_projected_effects"].(map[string]any)
	misplacedExisting["targets"].([]any)[0].(map[string]any)["source_identity"] =
		"smartrecruiters:example:42"
	if ProjectSemantics(misplaced)["reason"] != "invalid_projection" {
		t.Fatal("source identity outside JobEffect did not reject")
	}

	invalidExisting := newBase()
	invalidExistingInput := invalidExisting["input"].(map[string]any)
	invalidExistingEffects := invalidExistingInput["existing_projected_effects"].(map[string]any)
	invalidExistingEffects["job_effects"].([]any)[0].(map[string]any)["source_identity"] =
		"not-namespaced"
	if ProjectSemantics(invalidExisting)["reason"] != "invalid_projection" {
		t.Fatal("invalid existing JobEffect identity did not reject")
	}

	misplacedContent := newBase()
	jobsFor(misplacedContent)[0].(map[string]any)["content"].(map[string]any)["source_identity"] =
		"smartrecruiters:example:42"
	if ProjectSemantics(misplacedContent)["reason"] != "invalid_projection" {
		t.Fatal("source identity inside JobContent did not reject")
	}

	existingCollision := newBase()
	existingInput := existingCollision["input"].(map[string]any)
	existingEffects := existingInput["existing_projected_effects"].(map[string]any)
	collisionURL := "https://jobs.example.invalid/openings/source-identity-z"
	existingEffects["urls_to_upsert"] = append(
		existingEffects["urls_to_upsert"].([]any), collisionURL,
	)
	existingEffects["content_hashes"] = append(
		existingEffects["content_hashes"].([]any),
		existingEffects["content_hashes"].([]any)[0],
	)
	jobCopy, err := semanticsClone(existingEffects["job_effects"].([]any)[0])
	if err != nil {
		t.Fatal(err)
	}
	jobCopy.(map[string]any)["source_url"] = collisionURL
	existingEffects["job_effects"] = append(existingEffects["job_effects"].([]any), jobCopy)
	targetCopy, err := semanticsClone(existingEffects["targets"].([]any)[0])
	if err != nil {
		t.Fatal(err)
	}
	targetCopy.(map[string]any)["url"] = collisionURL
	existingEffects["targets"] = append(existingEffects["targets"].([]any), targetCopy)
	if ProjectSemantics(existingCollision)["reason"] != "canonical_collision" {
		t.Fatal("one identity on different existing URLs did not collide")
	}
}

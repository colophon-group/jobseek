...
`cases` contain the same 54 independently hard-coded IDs in the same order:
the original 50 byte-stable cases followed by exactly four source-identity
cases.
...
strings at their frozen v1 locations. Omission and explicit JSON null both
mean legacy canonical-URL identity and preserve the original 50 projections
and content/metadata hashes byte-for-byte. A present value must satisfy the
current-v1 bounded `provider:tenant:id` registry exactly and propagates
losslessly from a discovered job to its aligned `JobEffect`.

Source identity never enters `content_sha256` or
`metadata_updates_sha256`. Because a present identity is part of the projected
canonical result, it changes `semantic_sha256`. URL-only targets and scrape
projections remain identity-absent.

Identical duplicates require the same canonical URL, the same absent/present
identity state and value, and byte-identical canonical content. The same
present identity on different canonical URLs, the same canonical URL with
different present identities, or mixing absent and present identity for one
canonical URL is `canonical_collision`. Existing projected effects validate
the same location, registry, alignment, ordering, and collision rules.

Current production publication selection and mutable outbound-URL behavior
remain upstream dispatcher/persistence concerns. This offline lane neither
chooses among several outbound URLs for one identity nor creates alias or
...
standard-library, and zero-network. Both languages hard-code all 54 required

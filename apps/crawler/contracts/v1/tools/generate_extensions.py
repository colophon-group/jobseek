...
VALIDATORS = {"evaluation_json", "monitor_config", "runtime_metadata", "scraper_config"}
...
def load_rules() -> list[dict[str, object]]:
    ...
    if schema_ids != sorted(schema_ids) or len(schema_ids) != len(set(schema_ids)):
        raise ValueError("extension schema IDs must be unique and sorted")
    for rule in rules:
        ...
        if (
            ...
            or rule["validator"] not in VALIDATORS
        ):

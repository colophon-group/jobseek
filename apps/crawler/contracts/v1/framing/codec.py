...
def _decode_bounds(data: memoryview, maximum: int) -> tuple[int, int]:
    """Return the payload bounds after all prefix, cap, and length checks."""

    length, prefix_length = _decode_prefix(data)
    _check_cap(length, prefix_length, maximum)

    available = len(data) - prefix_length
    if available < length:
        _fail("truncated_payload", "record ended before its declared payload length")

    # length is now no greater than an existing Python buffer length, so this
    # addition cannot overflow or produce an out-of-range slice boundary.
    return prefix_length, prefix_length + length
...
def decode_next(data: bytes | bytearray | memoryview, maximum: int) -> tuple[bytes, memoryview]:
    """Decode the first record, returning its payload and a no-copy remainder."""

    _validate_maximum(maximum)
    view = _view(data, "data")
    start, end = _decode_bounds(view, maximum)
    return bytes(view[start:end]), view[end:]

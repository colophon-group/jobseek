...
type errorVector struct {
	...
	WireTrailingRepeatHex string `json:"wire_trailing_repeat_hex"`
	WireTrailingLength    int    `json:"wire_trailing_length"`
...
func vectorWire(t *testing.T, vector errorVector) []byte {
	t.Helper()
	wire, err := hex.DecodeString(vector.WireHex)
	if err != nil {
		t.Fatal(err)
	}
	if vector.WireTrailingRepeatHex == "" {
		return wire
	}
	unit, err := hex.DecodeString(vector.WireTrailingRepeatHex)
	if err != nil {
		t.Fatal(err)
	}
	return append(wire, bytes.Repeat(unit, vector.WireTrailingLength)...)
}
...
func TestSharedExactDecodeErrorVectors(t *testing.T) {
	for _, vector := range loadCorpus(t).Decode {
		vector := vector
		t.Run(vector.Name, func(t *testing.T) {
			_, decodeErr := DecodeRecord(vectorWire(t, vector), vector.Maximum)
...
func TestDecodeNextManyRecordsSharesBackingBuffer(t *testing.T) {
	const recordCount = 50_000
	wire := bytes.Repeat([]byte{0}, recordCount)
	remainder := wire
	for remainingCount := recordCount - 1; remainingCount >= 0; remainingCount-- {
		payload, next, err := DecodeNext(remainder, 1)
		if err != nil {
			t.Fatal(err)
		}
		if len(payload) != 0 || len(next) != remainingCount {
			t.Fatalf("got payload/remainder lengths %d/%d, want 0/%d", len(payload), len(next), remainingCount)
		}
		if len(next) != 0 && &next[0] != &wire[recordCount-remainingCount] {
			t.Fatal("remainder does not share the original backing buffer")
		}
		remainder = next
	}
}

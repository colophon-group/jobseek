...
trap cleanup_generation_before_start EXIT HUP INT TERM
...
python3 "$GENERATION/verify.py" compose "$GENERATION/compose.yml" "$GENERATION/release.env"
network_before_id=""
  ...
  if (( rollback_armed )); then
    ...
    if [[ -n "$previous_generation" ]]; then
      ...
      if [[ -n "$candidate_network_id" ]]; then
        ...
        python3 "$STAGE/verify.py" cleanup-network "$candidate_network_id"

 ...
 */
function buildScoringPrompt(signal: Signal, userProfile: UserProfile): string {
 ...
 */
function parseJsonResponse(text: string): ScoreResponse {
 ...
 */
function getDefaultScore(signalType: Signal['signal_type']): number {
  const defaults: Record<Signal['signal_type'], number> = {

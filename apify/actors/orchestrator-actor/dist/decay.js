...
exports.weeksElapsed = weeksElapsed;
...
function applyDecay(score, signalDate) {
    ...
    if (weeksElapsed <= 0)
        return score; // Future-dated signal — no decay
    // Exponential decay formula
    const decayFactor = Math.pow(1 - constants_1.SIGNAL_DECAY_RATE, weeksElapsed);
 ...
 */
function weeksElapsed(signalDate) {
    const now = new Date();
    const signal = new Date(signalDate);
    if (isNaN(signal.getTime()))
        return 0;

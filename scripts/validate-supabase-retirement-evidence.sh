...
validate_workflow_dispatch_input() {
  local run_id="$1" expected_mode="$2" archive actual_modes
  archive="${evidence_tmp}/run-${run_id}-logs.zip"
  if [[ ! -f "$archive" ]]; then
  ...
  test "$actual_modes" = "$expected_mode"
}

validate_run() {
  local run_id="$1" expected_path="$2" expected_name="$3" expected_title="$4" expected_event="$5" max_age="$6" expected_mode="$7"
  ...
  if [[ "$expected_mode" != - ]]; then
    ...
    validate_workflow_dispatch_input "$run_id" "$expected_mode"
  fi
...
latest_matching_run_id() {
  local workflow="$1" event="$2" expected_title="$3" expected_mode="$4"
  local records candidate_ids candidate_id
  records="$(gh api --method GET \
    "repos/${REPO}/actions/workflows/${workflow}/runs" \
    -f branch=main -f status=success -f event="$event" -f per_page=100)"
  candidate_ids="$(jq -r --arg title "$expected_title" \
    '.workflow_runs[] | select($title == "-" or .display_title == $title) | .id' \
    <<<"$records")"
  while IFS= read -r candidate_id; do
    [[ -n "$candidate_id" ]] || continue
    if [[ "$expected_mode" = - ]] \
      || validate_workflow_dispatch_input "$candidate_id" "$expected_mode"; then

...
VALID_MONITOR_TYPES='accenture|adp|almacareer|amazon|api_sniffer|ashby|avature|bamboohr|beehire|beisen|bite|brassring|breezy|bytedance|candidatus|cnstaff|comeet|computrabajo|cornerstone|curately|cvwarehouse|darwinbox|dayforce|deel|dom|dvinci|earcu|eightfold|fenbi|gem|greenhouse|gupy|headhunter|herp|hibob|hirehive|hireology|hrmos|icims|infoniqa|infor|inline|inploi|intervieweb|jarvi|jazzhr|job51|jobbank104|jobconvo|jobdiva|jobs_ch|jobstreet|jobvite|jobylon|johdi|join|keka|kipt|lever|linkedin|manatal|mokahr|nextdata|njoyn|notion|nowhiring|oracle_hcm|pageup|papa_johns|paycom|paylocity|paynet|peoplesoft|personio|phenom|pinpoint|practicematch|prospective|recruitee|recruiter_co_kr|recruiterbox|rippling|rss|seamlesshiring|seek|sitemap|smartrecruiters|softgarden|talemetry|talentbrew|talentreef|taleo|traffit|turbohire|typify|ukg|umantis|unifr|unisante|universia|wecruit|welcometothejungle|woowa|workable|workday|ycombinator'
VALID_SCRAPER_TYPES='adp|api_sniffer|bite|dom|eightfold|embedded|headhunter|infor|jazzhr|jobconvo|jobstreet|johdi|json-ld|linkedin|mokahr|nextdata|notion|onlyfy|oracle_hcm|paycom|paycor|paylocity|peoplesoft|pdf|phuketall|recruiterbox|rippling|seek|skip|smartrecruiters|taleo|tupu360|veryeast|workable|workday'
...
while IFS= read -r f; do
  ...
  if [ "$MATCH" != "true" ]; then
    case "$f" in
      ...
      apps/crawler/traces/traces.jsonl) MATCH=true ;;
    esac
  fi
  if [ "$MATCH" != "true" ]; then

...
1. Window: last 24h ending at UTC now, rounded down to the minute. Use an
   ...
   Always read the disk_capacity manifest entry and the six bounded files
   host/docker-system-df.txt, host/docker-container-sizes.txt,
   host/docker-images.txt, host/disk-attribution.txt,
   host/docker-gc-timer.txt, and host/docker-gc.log.
   ...
   Treat active(running) as a healthy in-flight timer and inspect its target
   service separately. A quiescent timer must be active(waiting) with a finite
   next trigger; active(elapsed), any other state, or waiting with a
   missing/infinite next trigger is a failed recurrence guard even while disk
   usage is safe.
